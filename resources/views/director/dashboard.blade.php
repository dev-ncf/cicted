    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
            ...
            <div class="h-20 flex items-center justify-center border-b border-unirovuma-800 bg-unirovuma-900">
                <div class="flex items-center gap-2">
                    ...
                    <span class="text-lg font-bold tracking-wide">CICTED <span
                            class="text-unirovuma-gold">DIRETOR</span></span>
                </div>
            </div>
            ...
            <div class="px-4 py-4 bg-unirovuma-800 border-b border-unirovuma-900">
                ...
                <p class="text-sm font-bold text-white mt-1 truncate">
                    @foreach (Auth::user()->thematicAreas as $area)
                        <i class="fas fa-tag mr-1 text-unirovuma-gold"></i> {{ $area->name }} <br>
                    @endforeach
                </p>
            </div>
            ...
            <nav class="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
                ...
                <a href="#" @click.prevent="currentTab = 'dashboard'; sidebarOpen = false"
                    :class="currentTab === 'dashboard' ? 'bg-unirovuma-800 border-r-4 border-unirovuma-gold text-white' :
                ...
                <a href="#" @click.prevent="currentTab = 'team'; sidebarOpen = false"
                    :class="currentTab === 'team' ? 'bg-unirovuma-800 border-r-4 border-unirovuma-gold text-white' :
                        'text-gray-300 hover:bg-unirovuma-800'"
                    class="flex items-center px-4 py-3 text-sm font-medium rounded-lg transition-all">
        ...
        <div class="flex-1 flex flex-col overflow-hidden relative">
            ...
            <main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-50 p-4 lg:p-8">
                ...
                @if (session('success'))
                    <div
                        class="mb-6 bg-green-50 border-l-4 border-green-500 p-4 rounded-r shadow-sm flex items-start animate-fade-in-down">
                ...
                <div x-show="currentTab === 'dashboard'" x-transition:enter="transition ease-out duration-300">
                    ...
                    <p class="text-gray-500 mb-6">Resumo da área: <span
                            class="font-semibold text-unirovuma-900">{{ Auth::user()->thematic_area }}</span></p>

                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                        <!-- Novos / Pendentes de Atribuição -->
                        <div
                            class="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex items-center justify-between border-l-4 border-l-red-500">
                            <div>
                                ...
                                @php
                                    ...
                                    foreach ($submissions ?? [] as $key) {
                                        # code...
                                        if ($key->status == 'submetido') {
                                            # code...
                                            $novasSub = +1;
                                        }
                                        if ($key->status == 'em_avaliacao') {
                                            # code...
                                            $emanalise = +1;
                                        }
                                        if (
                                            $key->status == 'aceite_com_correcoes' ||
                                            $key->status == 'aceite' ||
                                            $key->status == 'devolvido'
                                        ) {
                                            # code...
                                            $finalisados = +1;
                                        }
                                    ...
                                    //  dd($novasSub);
                                @endphp
                                <p class="text-3xl font-bold text-gray-800 mt-1">{{ $novasSub ?? 0 }}</p>
                            </div>
                            <div class="p-3 bg-red-50 text-red-600 rounded-lg"><i class="fas fa-inbox text-xl"></i>
                            </div>
                        </div>

                        <!-- Em Avaliação -->
                        <div
                            class="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex items-center justify-between border-l-4 border-l-yellow-500">
                            <div>
                                <p class="text-xs text-gray-500 font-bold uppercase">Em Andamento</p>
                                <p class="text-xs text-gray-400">Com avaliador definido</p>
                                <p class="text-3xl font-bold text-gray-800 mt-1">{{ $emanalise ?? 0 }}</p>
                            </div>
                            <div class="p-3 bg-yellow-50 text-yellow-600 rounded-lg"><i
                                    class="fas fa-user-clock text-xl"></i></div>
                        </div>

                        <!-- Concluídos -->
                        <div
                            class="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex items-center justify-between border-l-4 border-l-green-500">
                            <div>
                                <p class="text-xs text-gray-500 font-bold uppercase">Finalizados</p>
                                <p class="text-xs text-gray-400">Decisão tomada</p>
                                <p class="text-3xl font-bold text-gray-800 mt-1">{{ $finalisados ?? 0 }}</p>
                            </div>
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                        <div class="flex justify-between items-center mb-4">
                            ...
                            <button @click="currentTab = 'assignments'"
                                class="text-sm text-unirovuma-500 hover:text-unirovuma-900 font-medium">Ver
                                Todos</button>
                        </div>
                        <div class="overflow-x-auto">
                            <table class="w-full text-left text-sm">
                                ...
                                <tbody class="divide-y divide-gray-100">
                                    @forelse($submissions ?? [] as $reg)
                                        @if ($reg->status == 'submetido')
                                            <tr class="hover:bg-gray-50">
                                                <td class="px-4 py-3 font-medium text-gray-800 truncate max-w-xs">
                                                    {{ $reg->title }}</td>
                                                <td class="px-4 py-3 text-gray-600">{{ $reg->user->name }}</td>
                                                <td class="px-4 py-3 text-right">
                                                    <button
                                                        @click="showAssignModal = true; selectedReg = {{ json_encode($reg) }}"
                                                        class="bg-unirovuma-900 text-white px-3 py-1 rounded text-xs hover:bg-blue-800 transition">
                                                        Atribuir
                                                    </button>
                                                </td>
                                            </tr>
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="3" class="px-4 py-6 text-center text-gray-400 italic">Tudo
                                                em dia! Nenhum resumo pendente de atribuição.</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            </table>
                                ...
                                <tbody class="divide-y divide-gray-100">
                                    @forelse($submissions ?? [] as $reg)
                                        <tr class="hover:bg-gray-50 transition">
                                            <td class="px-6 py-4">
                                                <p class="font-bold text-gray-800 text-sm truncate w-64"
                                                    title="{{ $reg->title }}">{{ $reg->title }}</p>
                                                <p class="text-xs text-gray-500 mt-1">{{ $reg->user->name }}</p>
                                            </td>
                                            <td class="px-6 py-4">
                                                @if (!$reg->avaliador_id)
                                                    <span
                                            ...
                                            <td class="px-6 py-4">
                                                @if ($reg->avaliador_id)
                                                    <div class="flex items-center">
                                                        ...
                                                        <span
                                                            class="text-sm text-gray-700 font-medium">{{ $reg->user->name }}</span>
                                                    </div>
                                                @else
                                            ...
                                            <td class="px-6 py-4 text-right space-x-2">
                                                <button
                                                    @click="showDetailModal = true; selectedReg = {{ json_encode($reg) }}"
                                                    class="text-gray-400 hover:text-unirovuma-900" title="Ler Resumo">
                                                ...
                                                <button
                                                    ...
                                                    {{ !$reg->avaliador_id ? 'bg-unirovuma-900 text-white border-transparent hover:bg-blue-900' : 'bg-white text-gray-600 border-gray-300 hover:bg-gray-50' }}">
                                                    <i class="fas fa-user-tag mr-1"></i>
                                                    {{ !$reg->avaliador_id ? 'Atribuir' : 'Alterar' }}
                                                </button>
                                            </td>
                                        </tr>
                                    @empty
                                        <tr>
                                            <td colspan="4" class="px-6 py-12 text-center text-gray-400">Nenhum
                                                resumo nesta área temática.</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            </table>
                ...
                <div x-show="currentTab === 'team'" x-transition:enter="transition ease-out duration-300">
                    <h2 class="text-2xl font-bold text-gray-800 mb-6">Equipe de Avaliação</h2>
                    @foreach (Auth::user()->thematicAreas as $key)
                        <i class="fas fa-tag mr-1 text-unirovuma-gold"></i> {{ $key->name }} <br>
                    @endforeach
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        @forelse($avaliadores ?? [] as $evaluator)
                            <div
                                class="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex items-center gap-4">
                                <div
                                    class="w-12 h-12 rounded-full bg-green-100 text-green-600 flex items-center justify-center font-bold text-xl">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-blue-500 mt-1 bg-blue-50 inline-block px-2 py-0.5 rounded">
                                        Avaliador</p>
                                </div>
                            </div>
                        @empty
                            <div
                                class="col-span-3 text-center py-10 bg-white rounded-xl border border-dashed border-gray-300">
                                <p class="text-gray-500">Nenhum avaliador cadastrado nesta área temática.</p>
                                <p class="text-xs text-gray-400">Contacte o Administrador para adicionar utilizadores.
                                </p>
                            </div>
                        @endforelse
                    </div>
    ...
    <div x-show="showDetailModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" x-cloak>
        ...
        <div class="bg-white rounded-2xl shadow-xl w-full max-w-2xl overflow-hidden z-10 relative">
            <div class="bg-unirovuma-900 px-6 py-4 flex justify-between items-center">
                ...
                <button @click="showDetailModal = false" class="text-gray-300 hover:text-white"><i
            ...
            <div class="p-6 overflow-y-auto max-h-[70vh]">
                ...
                <div class="flex items-center gap-2 mb-4">
                    ...
                    <span class="text-sm text-gray-800" x-text="selectedReg?.user.name"></span>
                </div>
                <div class="p-4 bg-gray-50 rounded-lg border border-gray-100 text-sm text-justify leading-relaxed whitespace-pre-line"
                    x-text="selectedReg?.abstract_content"></div>
                ...
                <div class="mt-4 flex gap-3">
                    <template x-if="selectedReg?.abstract_filepath">
                        <a :href="'{{ asset('') }}' + selectedReg?.abstract_filepath" target="_blank"
                            ...
                            <i class="fas fa-file-word mr-1"></i> Abrir Documento
                        </a>
                    </template>
                </div>
            </div>
            <div class="bg-gray-50 px-6 py-4 flex justify-end gap-3">
                <button @click="showDetailModal = false"
                    class="px-4 py-2 border rounded text-gray-600 hover:bg-white">Fechar</button>
                <button @click="showDetailModal = false; showAssignModal = true"
                    class="px-4 py-2 bg-unirovuma-900 text-white rounded hover:bg-blue-900 shadow">
                    Atribuir Avaliador
                </button>
            </div>
        </div>
    ...
    <div x-show="showAssignModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" x-cloak>
        ...
        <div class="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden z-10 relative">
            <div class="bg-gray-50 px-6 py-4 border-b border-gray-100">
                ...
                <p class="text-xs text-gray-500 mt-1">Selecione um especialista da área @foreach (Auth::user()->thematicAreas as $key)
            ...
            <form action="{{ route('abstracts.assign') }} " method="POST" class="p-6">
                @csrf
                <input type="hidden" name="submission_id" :value="selectedReg?.id">

                <div class="mb-2">
                    <label class="block text-xs font-bold text-gray-500 uppercase mb-2">Resumo</label>
                    <p class="text-sm font-medium text-gray-800 truncate p-2 bg-gray-50 rounded border border-gray-200"
                        x-text="selectedReg?.title"></p>
                </div>
                ...
                <div class="mt-6">
                    ...
                    <div class="relative">
                        <select name="avaliador_id" required
                            class="block w-full rounded-lg border-gray-300 py-3 px-4 shadow-sm focus:border-unirovuma-500 focus:ring-unirovuma-500 sm:text-sm border bg-white">
                ...
                <div class="mb-2">
                    <label class="block text-xs font-bold text-gray-500 uppercase mb-2">Prazo</label>
                    <input name="prazo" type="date" required
                        class="text-sm font-medium text-gray-800 truncate p-2 bg-gray-50 rounded border border-gray-200">
                </div <div class="mt-8 flex justify-end gap-3">
                ...
                <button type="submit"
                    class="px-5 py-2 bg-unirovuma-900 text-white font-bold rounded-lg hover:bg-blue-900 shadow text-sm">Confirmar
                    Atribuição</button>
        </div>
        </form>
    </div>
